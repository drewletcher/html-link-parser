    ...
    try {
      ...
      saxStream.on("closetag", function (tag) {
        ...
        if (isHeading(tag)) {
          ...
          if (findHeading && !foundHeading && compareText(headingText, self.options.heading)) {
            foundHeading = true;
            headingTag = tag;
          }
        }

        if (foundHeading && self.options.terms.length === 0) {
          // all links in document
          foundLink = true;
        }

        if (inAnchor && foundHeading && !foundLink) {
        ...
        if (tag === "A" && foundHeading && !foundLink) {
