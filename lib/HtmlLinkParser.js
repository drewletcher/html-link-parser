...
export default class HtmlLinkParser extends EventEmitter {
  ...
  async parse(url) {
    try {
      ...
      // heading filters
      let findTag = Object.hasOwn(this.options, "tag") && this.options.tag;
      let findHeading = Object.hasOwn(this.options, "heading") && this.options.heading;
      ...
      // current values
      let redirect = 0;
      let location = url || this.options.url;

      let inTitle = false;
      let titleText = "";
      let robotsText = "";  // from HTTP header X-Robots-Tag and/or <meta name="robots">
      ...
      else if (typeof location === "string" && !location.toLowerCase().startsWith("http")) {
        rs = fs.createReadStream(location);
      }
      else {
        rs = await httpRequest.createReadStream(location, this.options.http);

        let redirectCount = 0;
        while ([ 301, 302, 307, 308 ].includes(rs.statusCode)) {
          ++redirectCount;
          if (redirectCount > 10) {
            throw new Error("Too many redirects");
          }

          redirect = rs.statusCode;
          location = rs.headers.location;
          rs.resume();  // drain the stream
          rs = await httpRequest.createReadStream(location, this.options.http);
        }

        if (rs.statusCode === 200) {
          if (Object.hasOwn(rs.headers, "x-robots-tag"))
            robotsText += rs.headers[ "x-robots-tag" ] + ",";
        }
        else {
          rs.resume();  // drain the stream
          throw new Error(rs.statusCode + " " + rs.statusMessage + " " + location);
        }
      }
      ...
      saxStream.on("opentag", function (node) {
        ...
        if (tag === "A") {
          ...
          if (Object.hasOwn(node.attributes, "name") && node.attributes.name === "robots")
            robotsText += node.attributes.content + ",";
        }
      });
      ...
      saxStream.on("closetag", function (tag) {
        ...
        if (tag === "TD" & inTableRow) {
          // separate table cells with tabs
          tableText += "";
        }
        if (tag === "TR") {
          inTableRow = false;
        ...
        if (inAnchor && foundLink) {
          ...
          if (link.href && !link.href.startsWith("http") && location.startsWith("http")) {
            // expand relative URLs to absolute based on page location
            try {
              link.href = decodeURI(new URL(link.href, location).href);
            }
            catch (err) {
              // ignore invalid URLs
            }
