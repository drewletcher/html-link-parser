...
export default class HtmlLinkParser extends EventEmitter {
  ...
  async parse() {
    try {
      ...
      let findHref = Object.hasOwn(this.options, "href");
      let findText = Object.hasOwn(this.options, "text");
      let foundLink = false;

      //// table processing
      const tagStack = []; // track state of tag open elements
      const currTag = () => {
      ...
      saxStream.on("opentag", function (node) {
        ...
        switch (node.name) {
          case "A":
            ...
            if (!findRefId && !findHref && !findText) {
              foundLink = true;
      ...
      saxStream.on("closetag", function (tag) {
        ...
        if (self.isHeading(tag)) {
          ...
          else if (findHeading && self.compareText(self.options.heading, headingText)) {
